  ...
  setValues: function(formName, obj) {
    ...
    for (key in obj) {
      ...
      if (typeof obj[key] === 'object') {
        this._.setValuesOfNestedFields(formName, key, obj[key])
      }
      else if (typeof obj[key] === 'array') {
        obj[key].forEach(function(element, index) {
          this._.setValuesOfNestedFields(formName, key + '/' + index, element)
        })
      }
      else {
        this._.virtualForms[formName][key] = obj[key];
      }
    }
  },
...
Cape.extend(_Internal.prototype, {
  getValue: function(name) {
    ...
    if (form = this.forms[formName]) {
      _form = {};
      this.flatten(_form, form);
      if (_form[attrName] !== undefined) return _form[attrName];
    }
    return '';
  },
  ...
  setValuesOfNestedFields: function(formName, name, obj) {
    for (key in obj) {
      if (typeof obj[key] === 'object') {
        this.setValuesOfNestedFields(formName, name + '/' + key, obj[key])
      }
      else if (typeof obj[key] === 'array') {
        obj[key].forEach(function(element, index) {
          this.setValuesOfNestedFields(formName, name + '/' + key + '/' + index, element)
        })
      }
      else {
        this.virtualForms[formName][name + '/' + key] = obj[key];
      }
    }
  },
  flatten: function(_obj, obj, name) {
    if (name) {
      name = name + '/'
    }
    else {
    ...
    for (key in obj) {
      if (typeof obj[key] === 'object') {
        this.flatten(_obj, obj[key], name + key)
      }
      else {
        _obj[name + key] = obj[key];
      }
    }
  },
